...
export const MovieDetail = () => {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=eed2deb358543780fe190c34b31aa059`)
            ...
            movie.genres ? (
              <p className="my-7 flex flex-wrap gap-2">
                {movie.genres.map((genre) => (
                  <span className="bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 p-2" key={genre.id}>{genre.name}</span>
                ))}
              </p>) : ""
          }
